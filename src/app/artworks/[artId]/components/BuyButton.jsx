...
export default function BuyButton({ artworkId }) {
  ...
  const handleConfirmPurchase = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setToastConfig({
          type: "success",
          message: data.message || "Artwork purchased successfully!",
        });
        setShowToast(true);
        router.refresh();
      }
  ...
  return (
    <>
      <button 
        ...
        disabled={purchasing}
        className="w-full h-11 px-5 inline-flex items-center justify-center bg-[#C2693F] text-[#F7F4EF] text-sm font-['DM_Sans'] font-medium rounded-[6px] hover:bg-[#A3522E] transition-colors duration-200 cursor-pointer shadow-sm disabled:opacity-55"
      >
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-[#EDE9E1] border border-[#D6CFC4] rounded-[12px] p-6 max-w-sm w-full shadow-2xl animate-in fade-in zoom-in-95 duration-200 font-['DM_Sans']">
            ...
            <p className="text-sm text-[#6B6560] leading-relaxed mb-6">
              Are you sure you want to purchase this artwork? Confirming will add it to your collection.
            </p>
